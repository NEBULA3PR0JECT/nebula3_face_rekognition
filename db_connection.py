...
class WeaviateDB:
    ...
    def search(self, vec="", limit=1):
        ...
        if vec != "":
            ...
            near_vec = {"vector": vec}
            res = self.client \
                .query.get("dataset_wiki_imdb", ["actor_name"]) \
                .with_near_vector(near_vec) \
                .with_limit(limit) \
                .do()
            search_took = time.time() - before
            actor_name = res['data']['Get']['Dataset_wiki_imdb'][0]['actor_name']
            print("Time it took to search: {:.4f}, Actor name: ".format(search_took, actor_name))
...
def main():
    ...
    # UPLOAD WIKI TO WEAVIATE DB
    # data_path =  "/notebooks/nebula3_face_rekognition/actor_name_to_cluster_wiki_all.pkl" #"/notebooks/nebula3_face_rekognition/actor_name_to_cluster_combined.pkl"
    # data = imdb_instance.read_data(data_path, mode="pickle")
    # db_instance.import_face_vectors(data)

    # UPLOAD IMDB TO WEAVIATE DB
    # data_path = "/notebooks/nebula3_face_rekognition/actor_name_to_cluster_combined.pkl"
    # data = imdb_instance.read_data(data_path, mode="pickle")
    # db_instance.import_face_vectors(data)
